        ...
        handlers._getMenuItems((err, menu) => {
          if (!err && menu) {
            ...
            stripe.pay({
              ...
              // Log current order
              handlers.logOrder(email, data, err, totalCart);
            });
          } else {
            callback(403, {error: err})
          }
...
// Log current order
handlers.logOrder = (email, data = {}, err, cart) => {
  // Form the log data
  ...
  const logData = {
    ...
    cardBrand: data.source.brand,
    cardCountry: data.source.country,
    email,
    data,
    cart,
    date: +new Date(),
  ...
  _logs.append(`orders/${logData.id}_${helpers.getCurrentDate()}`, logString, err => {
    if(!err){
...
handlers.logSignup = (email) => {
  ...
  // Append the log string to the file
  _logs.append(`signup/${helpers.getCurrentDate()}`, logString, err => {
    if(!err){
      debug("Logging to file succeeded");
    } else {
      debug("Logging to file failed");
